...
Public Function LoadHighlighter(strFile As String)
  ...
  For i = 0 To 127
    ...
    If S <> "" Then
      ...
      If UCase(d(11)) <> "" Then Highlighters(HCount).StyleName(i) = d(11)
      Erase d
    Else
      S = ReadINI("data", "style[" & 32 & "]", strFile)
      d = Split(S, ":")
      If UCase(d(0)) = "B" Then Highlighters(HCount).StyleBold(i) = 1
      If UCase(d(1)) = "I" Then Highlighters(HCount).StyleItalic(i) = 1
      If UCase(d(2)) = "U" Then Highlighters(HCount).StyleUnderline(i) = 1
...
Public Function ExportToHTML(strFile As String, cScintilla As clsScintilla)
  ...
  For i = 0 To iLen
    ...
    If curStyle <> prevStyle Then
      ...
      If cScintilla.GetCharAtLong(i) <> 13 And cScintilla.GetCharAtLong(i) <> 60 And cScintilla.GetCharAtLong(i) <> 62 Then
      ...
      Else
        If cScintilla.GetCharAtLong(i) = 13 Then
          SConcat "<BR>"
        ElseIf cScintilla.GetCharAtLong(i) = 60 Then
          SConcat "&LT;"
        ElseIf cScintilla.GetCharAtLong(i) = 62 Then
          SConcat "&GT;"
        End If
...
Public Sub DoSyntaxOptions(strDir As String, frm As Form)
  Load frmOptions
  With frmOptions
    .ListLangs strDir
    .Show vbModal, frm
...
Public Sub ResetSyntaxMDI(strDocName As String)
  ...
  For Each frm In Forms
    If frm.Name = strDocName And frm.Visible = True Then
      SetHighlighter frm.sciMain, Highlighters(frm.sciMain.lHighlighterNum).strName
    End If
  Next
End Sub
